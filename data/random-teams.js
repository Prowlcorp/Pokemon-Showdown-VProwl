...
class RandomTeams extends Dex.ModdedDex {
	...
	randomCCTeam() {
		...
		for (let i = 0; i < 6; i++) {
			...
			// Random shininess
			let shiny = this.randomChance(1, 1500);
	...
	queryMoves(moves, hasType = {}, hasAbility = {}, movePool = []) {
		...
		let RecoveryMove = [
			'healorder', 'milkdrink', 'moonlight', 'morningsun', 'recover', 'roost', 'softboiled', 'synthesis',
		];
	...
	randomSet(template, slot = 1, teamDetails = {}, isDoubles = false) {
		...
		return {
			...
			item: item,
			level: level,
			shiny: this.randomChance(1, 1500),
		};
	}
	...
	randomFactorySet(template, slot, teamData, tier) {
		...
		return {
			...
			shiny: typeof setData.set.shiny === 'undefined' ? this.randomChance(1, 1500) : setData.set.shiny,
