...
exports.commands = {
	...
	dexsearchhelp: [
		...
		`Valid tiers are: Uber/OU/UUBL/UU/RUBL/RU/NUBL/NU/PUBL/PU/ZU/NFE/LC Uber/LC.`,
...
function runDexsearch(target, cmd, canAll, message) {
	let searches = [];
	let allTiers = {'uber': 'Uber', 'ubers': 'Uber', 'ou': 'OU', 'uubl': 'UUBL', 'uu': 'UU', 'rubl': 'RUBL', 'ru': 'RU', 'nubl': 'NUBL', 'nu': 'NU', 'publ': 'PUBL', 'pu': 'PU', 'zu': '(PU)', 'nfe': 'NFE', 'lcuber': 'LC Uber', 'lcubers': 'LC Uber', 'lc': 'LC', __proto__: null};
	...
	for (const andGroup of target.split(',')) {
		...
		for (const parameter of parameters) {
			...
			if (target === 'recovery') {
				...
				let recoveryMoves = ["recover", "roost", "moonlight", "morningsun", "synthesis", "milkdrink", "softboiled", "wish", "healorder", "shoreup"];
	...
	for (let pokemon in mod.data.Pokedex) {
		...
		if (template.gen <= maxGen && template.tier !== 'Unreleased' && template.tier !== 'Illegal' && megaSearchResult) {
...
function runMovesearch(target, cmd, canAll, message) {
	...
	let allStatus = ['psn', 'tox', 'brn', 'par', 'frz', 'slp', 'bld'];
